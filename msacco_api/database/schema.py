...
class DBTable:
	...
	def get_column_definitions(self):
		column_list = [] + frappe.cbs_db.DEFAULT_COLUMNS
		ret = []
	...
	def get_index_definitions(self):
		...
		for key, col in self.columns.items():
			if (
				col.set_index
				and not col.unique
				and col.fieldtype in frappe.cbs_db.type_map
				and frappe.cbs_db.type_map.get(col.fieldtype)[0] not in ("text", "longtext")
			):
	...
	def get_columns_from_docfields(self):
		...
		if not self.meta.get("istable"):
			for fieldname in frappe.cbs_db.OPTIONAL_COLUMNS:
	...
	def validate(self):
		...
		columns = [
			frappe._dict({"fieldname": f, "fieldtype": "Data"}) for f in frappe.cbs_db.STANDARD_VARCHAR_COLUMNS
		]
		if self.meta.get("istable"):
			columns += [
				frappe._dict({"fieldname": f, "fieldtype": "Data"}) for f in frappe.cbs_db.CHILD_TABLE_COLUMNS
			]
		...
		for col in columns:
			...
			if "varchar" in frappe.cbs_db.type_map.get(col.fieldtype, ()):

				# validate length range
				new_length = cint(col.length) or cint(frappe.cbs_db.VARCHAR_LEN)
				...
				if cint(current_length) != cint(new_length):
					try:
						...
						max_length = frappe.cbs_db.sql(
							"""SELECT MAX(CHAR_LENGTH(`{fieldname}`)) FROM `tab{doctype}`""".format(
								fieldname=col.fieldname, doctype=self.doctype
							)
						)

					except frappe.cbs_db.InternalError as e:
						if frappe.cbs_db.is_missing_column(e):
	...
	def is_new(self):
		return self.table_name not in frappe.cbs_db.get_tables()

	def setup_table_columns(self):
		# TODO: figure out a way to get key data
		for c in frappe.cbs_db.get_table_columns_description(self.table_name):
...
class DbColumn:
	...
	def get_definition(self, with_default=1):
		...
		if self.fieldtype in ("Check", "Int"):
			...
			self.default
			and (self.default not in frappe.cbs_db.DEFAULT_SHORTCUTS)
			and not cstr(self.default).startswith(":")
			and column_def not in ("text", "longtext")
		):
			column_def += f" default {frappe.cbs_db.escape(self.default)}"
...
def validate_column_name(n):
	if special_characters := SPECIAL_CHAR_PATTERN.findall(n):
		...
		frappe.throw(
			...
			frappe.cbs_db.InvalidColumnName,
		)
	return n


def validate_column_length(fieldname):
	if len(fieldname) > frappe.cbs_db.MAX_COLUMN_LENGTH:
...
def get_definition(fieldtype, precision=None, length=None):
	d = frappe.cbs_db.type_map.get(fieldtype)
	...
	if fieldtype == "Int" and length and length > 11:
		...
		d = frappe.cbs_db.type_map.get("Long Int")
...
def add_column(
	...
	if column_name in frappe.cbs_db.get_table_columns(doctype):
		# already exists
		return
