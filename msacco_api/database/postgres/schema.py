...
class PostgresTable(DBTable):
	def create(self):
		varchar_len = frappe.cbs_db.VARCHAR_LEN
		...
		if (
			...
			frappe.cbs_db.create_sequence(self.doctype, check_not_exists=True, cache=frappe.cbs_db.SEQUENCE_CACHE)
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			frappe.cbs_db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = f"{frappe.cbs_db.escape(col.default)}"
		...
		try:
			if query:
				...
				# nosemgrep
				frappe.cbs_db.sql(final_alter_query)
			if create_contraint_query:
				# nosemgrep
				frappe.cbs_db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				frappe.cbs_db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if frappe.cbs_db.is_duplicate_fieldname(e):
				frappe.throw(str(e))
			elif frappe.cbs_db.is_duplicate_entry(e):
