...
from msacco_api.query_builder import Criterion, Field, Order, Table, functions
from msacco_api.query_builder.functions import Function, SqlFunctions
from msacco_api.query_builder.utils import PseudoColumn
...
def like(key: Field, value: str) -> frappe.cbs_qb:
    ...
    Returns:
            frappe.cbs_qb: `frappe.cbs_qb object with `LIKE`
    """
    return key.like(value)


def func_in(key: Field, value: list | tuple) -> frappe.cbs_qb:
    ...
    Returns:
            frappe.cbs_qb: `frappe.cbs_qb object with `IN`
    """
...
def not_like(key: Field, value: str) -> frappe.cbs_qb:
    ...
    Returns:
            frappe.cbs_qb: `frappe.cbs_qb object with `NOT LIKE`
    """
...
def func_not_in(key: Field, value: list | tuple | str):
    ...
    Returns:
            frappe.cbs_qb: `frappe.cbs_qb object with `NOT IN`
    """
    if isinstance(value, str):
        value = value.split(",")
...
def func_regex(key: Field, value: str) -> frappe.cbs_qb:
    ...
    Returns:
            frappe.cbs_qb: `frappe.cbs_qb object with `REGEX`
    """
    return key.regex(value)


def func_between(key: Field, value: list | tuple) -> frappe.cbs_qb:
    ...
    Returns:
            frappe.cbs_qb: `frappe.cbs_qb object with `BETWEEN`
    """
...
def func_timespan(key: Field, value: str) -> frappe.cbs_qb:
    ...
    Returns:
            frappe.cbs_qb: `frappe.cbs_qb object with `TIMESPAN`
    """
...
def table_from_string(table: str) -> "DocType":
    ...
    return frappe.cbs_qb.DocType(table_name=table_name.replace("`", ""))
...
class Engine:
    ...
    def OPERATOR_MAP(self):
        ...
        if frappe.get_hooks("filters_config"):
            ...
            warn(
                ...
                " in frappe.cbs_db.query engine. Use db_query (frappe.get_list) instead."
            )

        return all_operators

    def get_condition(self, table: str | Table, **kwargs) -> frappe.cbs_qb:
        ...
        Returns:
                frappe.cbs_qb: DocType with initial condition
        """
        table_object = self.get_table(table)
        if kwargs.get("update"):
            return frappe.cbs_qb.update(table_object)
        if kwargs.get("into"):
            return frappe.cbs_qb.into(table_object)
        return frappe.cbs_qb.from_(table_object)
    ...
    def get_table(self, table_name: str | Table) -> Table:
        ...
        if table_name not in self.tables:
            self.tables[table_name] = frappe.cbs_qb.DocType(table_name)
        return self.tables[table_name]

    def criterion_query(
        self, table: str, criterion: Criterion, **kwargs
    ) -> frappe.cbs_qb:
        ...
        Returns:
                frappe.cbs_qb: condition object
        """
        condition = self.add_conditions(self.get_condition(table, **kwargs), **kwargs)
        return condition.where(criterion)

    def add_conditions(self, conditions: frappe.cbs_qb, **kwargs):
        """Adding additional conditions

        Args:
                conditions (frappe.cbs_qb): built conditions

        Returns:
                conditions (frappe.cbs_qb): frappe.cbs_qb object
        """
        if kwargs.get("orderby") and kwargs.get("orderby") != "KEEP_DEFAULT_ORDERING":
            ...
            if isinstance(orderby, str) and len(orderby.split()) > 1:
                ...
                conditions = conditions.orderby(
                    orderby, order=kwargs.get("order") or Order.desc
                )
            ...
            for f in filters:
                if isinstance(f, (list, tuple)):
                    ...
                    if not isinstance(filters[0], str):
                        conditions = self.make_function_for_filters(
                            filters[0], filters[2]
                        )
                        break
                    conditions = conditions.where(
                        _operator(Field(filters[0]), filters[2])
                    )
                    break
    ...
    def dict_query(
        self, table: str, filters: dict[str, str | int] = None, **kwargs
    ) -> frappe.cbs_qb:
        ...
        Returns:
                frappe.cbs_qb: conditions object
        """
        conditions = self.get_condition(table, **kwargs)
        if isinstance(table, str):
            table = frappe.cbs_qb.DocType(table)
        if not filters:
        ...
        filters = {
            (self.get_function_object(k) if has_function(k) else k): v
        ...
        for key in filters:
            ...
            if not isinstance(key, str):
                conditions = conditions.where(
                    self.make_function_for_filters(key, value)
                )
                continue
    ...
    def build_conditions(
        ...
        Returns:
                frappe.cbs_qb: frappe.cbs_qb conditions object
        """
            ...
            if to_cast:
                ...
                field = (
                    (
                        ...
                        else PseudoColumn(initial_fields)
                    )
        ...
        for function in function_objects:
            if isinstance(fields, str):
                ...
                fields = BRACKETS_PATTERN.sub(
                    "", re.sub(function.name, "", fields, flags=re.IGNORECASE)
                )
                ...
                for field in fields:
                    if isinstance(field, str):
                        ...
                        if substituted_string.casefold() == function.name.casefold():
                            replaced_string = substituted_string.casefold().replace(
                                function.name.casefold(), ""
                            )
                        else:
                            replaced_string = substituted_string.replace(
                                function.name.casefold(), ""
                            )
    ...
    def get_fieldnames_from_child_table(self, doctype, fields):
        ...
        for field in fields:
            if "." in field and "tab" not in field:
                ...
                linked_field = frappe.get_meta(doctype, cached=True).get_field(
                    self.fieldname
                )
                try:
    ...
    def sanitize_fields(self, fields: str | list | tuple):
        is_mariadb = frappe.cbs_db.db_type == "mariadb"
    ...
    def set_fields(self, fields, **kwargs) -> list:
        ...
        if is_str and "," in fields:
            fields = [
                field.replace(" ", "") if "as" not in field else field
                ...
                if (
                    ...
                    if self.fieldname:
                        criterion = criterion.left_join(join_table).on(
                            getattr(join_table, "name")
                            == getattr(frappe.cbs_qb.DocType(table), self.fieldname)
                        )
                    else:
                        criterion = criterion.left_join(join_table).on(
                            getattr(join_table, "parent")
                            == getattr(frappe.cbs_qb.DocType(table), "name")
                        )
                    has_join = True
                ...
                def _update_pypika_fields(field):
                    if not is_pypika_function_object(field):
                        ...
                        if not TABLE_PATTERN.search(str(field)):
                            return getattr(frappe.cbs_qb.DocType(table), field)
                    else:
                        field.args = [
                            getattr(frappe.cbs_qb.DocType(table), arg.get_sql())
    ...
    def get_query(
        ...
        **kwargs,
        ...
        join = (
            kwargs.get("join").replace(" ", "_") if kwargs.get("join") else "left_join"
        )
        criterion, fields = self.join_(
            criterion=criterion, fields=fields, table=table, join=join
        )
...
class Permission:
    ...
    def get_tables_from_query(query: str):
        return [
            table for table in WORDS_PATTERN.findall(query) if table.startswith("tab")
        ]
