...
from msacco_api.query_builder.terms import NamedParameterWrapper
...
def get_qb_engine():
    from msacco_api.database.query import Engine
...
def DocType(*args, **kwargs):
    return frappe.cbs_qb.DocType(*args, **kwargs)


def Table(*args, **kwargs):
    return frappe.cbs_qb.Table(*args, **kwargs)


def patch_query_execute():
    """Patch the Query Builder with helper execute method
    This excludes the use of `frappe.cbs_db.sql` method while
    executing the query object
    """

    def execute_query(query, *args, **kwargs):
        query, params = prepare_query(query)
        return frappe.cbs_db.sql(query, params, *args, **kwargs)  # nosemgrep
    ...
    query_class = get_attr(str(frappe.cbs_qb).split("'")[1])
    ...
    frappe._cbs_qb_patched[frappe.conf.db_type] = True


def patch_query_aggregation():
    """Patch aggregation functions to frappe.cbs_qb"""
    from frappe.query_builder.functions import _avg, _max, _min, _sum

    frappe.cbs_qb.max = _max
    frappe.cbs_qb.min = _min
    frappe.cbs_qb.avg = _avg
    frappe.cbs_qb.sum = _sum
    frappe._cbs_qb_patched[frappe.conf.db_type] = True
